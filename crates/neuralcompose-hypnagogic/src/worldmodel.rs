...
/// Seeds for the §8 held-out run, disjoint from [`DEFAULT_SEEDS`].
///
/// **These refresh MPPI and nothing else.** `PdController` and `BangBang` are
/// deterministic: a scenario yields one trajectory, and five more seeds give
/// five identical copies rather than five samples. A seed-based replication
/// would therefore re-test one arm of three while appearing to re-test all of
/// them — which is why [`heldout_cases`] exists alongside this.
pub const HELDOUT_SEEDS: [u64; 5] = [5, 6, 7, 8, 9];

/// The §8 held-out scenarios: antipodal pairs at radius 0.8, every 45°.
///
/// **Generated by rule, not chosen by inspection.** That is the whole point —
/// the step metric was selected after seeing it separate on `HARD_CASES`, so the
/// replication has to run on data that did not inform the choice. None of these
/// is a `HARD_CASES` member: those corners sit at radius `0.8·√2 ≈ 1.131`, while
/// every point here is at radius `0.8` exactly.
pub fn heldout_cases() -> Vec<Scenario> {
    const NAMES: [&str; 8] = [
        "held_000", "held_045", "held_090", "held_135", "held_180", "held_225", "held_270",
        "held_315",
    ];
    (0..8)
        .map(|k| {
            let theta = (k as f32) * std::f32::consts::FRAC_PI_4;
            let (sx, sy) = (0.8 * theta.cos(), 0.8 * theta.sin());
            Scenario {
                name: NAMES[k],
                start: [sx, sy],
                goal: [-sx, -sy],
...
mod planner_tests {
    ...
    /// The held-out set must not overlap the set the metric was chosen on, or
    /// the §8 replication is measuring the same data twice.
    #[test]
    fn the_heldout_scenarios_are_disjoint_from_the_hard_cases() {
        let held = heldout_cases();
        assert_eq!(held.len(), 8);
        for h in &held {
            let r = (h.start[0] * h.start[0] + h.start[1] * h.start[1]).sqrt();
            assert!(
                (r - 0.8).abs() < 1e-5,
                "{} is at radius {r}, not 0.8",
                h.name
            );
            // Antipodal by construction.
            assert!((h.goal[0] + h.start[0]).abs() < 1e-6);
            assert!((h.goal[1] + h.start[1]).abs() < 1e-6);
            for hc in HARD_CASES.iter() {
                let same = (h.start[0] - hc.start[0]).abs() < 1e-4
                    && (h.start[1] - hc.start[1]).abs() < 1e-4;
                assert!(!same, "{} coincides with {}", h.name, hc.name);
            }
        }
        // And the seeds are disjoint too.
        for s in HELDOUT_SEEDS {
            assert!(!DEFAULT_SEEDS.contains(&s), "seed {s} is not held out");
        }
    }
}
