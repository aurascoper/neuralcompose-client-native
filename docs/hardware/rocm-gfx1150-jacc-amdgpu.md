...
4. **Ubuntu's ROCm math libraries are broken for gfx1150 in three distinct ways** (packaging gaps, not Julia bugs): rocRAND **segfaults** (ships gfx1151 kernels but not gfx1150), rocBLAS **errors then aborts** (~~ships only `TensileLibrary_lazy_gfx942.dat` — datacenter-only GEMM kernels~~ — corrected 2026-08-04, same day: the full list was in the captured log all along and the claim was built from a `head`-truncated display; rocBLAS ships `TensileLibrary_lazy_*.dat` for **nine** targets — gfx908/90a/942/1030/1100/1101/1151/1200/1201 — and omits exactly gfx1150), rocSPARSE **segfaults in `rocsparse_create_handle`** (no gfx115x objects at all). The pattern across all three, and across rocBLAS's `Kernels.so-*.hsaco` set: **gfx1151 (Strix Halo) is built, gfx1150 (Strix Point) is omitted.** All three now have Julia-free ~10-line C reproductions compiled with the distro's own hipcc (see §1).
...
| rocBLAS `mul!` | error: `Cannot read /usr/lib/x86_64-linux-gnu/rocblas/5.1.0/library/TensileLibrary.dat: Illegal seek for GPU arch : gfx1150`; ~~only available file is `TensileLibrary_lazy_gfx942.dat`~~ the log lists lazy `.dat` files for gfx942/90a/908/1030/1200/1151 — **none for gfx1150** (claim corrected same day; built from a `head`-truncated display of this very log) | log `amdgpu-jl-rocblas.txt` |
...
| rocRAND C repro (Julia-free) | **SIGSEGV, exit 139**, before any output: `rocrand_create_generator`/`generate_uniform` on default pseudo generator | `hipcc repro-rocrand.cpp -lrocrand` (10-line C, evidence dir) |
| rocBLAS C repro (Julia-free) | `create_handle` OK, then 4×4 `rocblas_sgemm` → same TensileLibrary error (errno text `No such file or directory` in this context) and **SIGABRT, exit 134**; full list shows nine lazy `.dat` targets incl. gfx1151, no gfx1150 | `hipcc repro-rocblas.cpp -lrocblas` (evidence dir) |
| rocSPARSE C repro (Julia-free) | **SIGSEGV, exit 139**, inside `rocsparse_create_handle`, before any output | `hipcc repro-rocsparse.c -lrocsparse` (evidence dir) |
| rocBLAS on-disk kernel inventory | `Kernels.so-000-<arch>.hsaco` for gfx1030/1100/1101/**1151**/1200/1201/908/90a/942 — no gfx1150; Tensile `.co`/`.dat` same nine | `ls /usr/lib/x86_64-linux-gnu/rocblas/5.1.0/library/` |
...
| librocblas gfx11 targets (host lib) | includes `gfx1150` — but Tensile GEMM kernel *files* on disk ~~are gfx942-only~~ cover nine targets incl. gfx1151 and omit exactly gfx1150 (corrected same day) | heuristic + rocBLAS error text + `ls` of the library dir |
