...
/// Windowed-periodogram band power over `samples` at `fs` Hz.
///
/// Returns `0.0` when there is less than one second of data, matching the
/// Python guard `if len(signal_1d) < int(fs)`. That is a deliberate refusal
/// rather than a small-sample estimate: a sub-second window cannot resolve an
/// 8-13 Hz band well enough for the ratio to mean anything.
///
/// Returns `0.0` when no FFT bin falls inside the band, and when any sample is
/// non-finite — the Python version has no such guard and would propagate NaN
/// silently through a ratio, which would then compare as `false` against a
/// threshold and read as a quiet failure rather than a loud one.
pub fn band_power(samples: &[f64], fs: f64, band: (f64, f64)) -> f64 {
    let n = samples.len();
...
mod tests {
    ...
    fn a_tone_inside_the_band_dominates_a_tone_outside_it() {
        let n = 2048;
        let inside = band_power(&sine(10.0, 20.0, n), FS, ALPHA);
        let outside = band_power(&sine(22.0, 20.0, n), FS, ALPHA);
        assert!(
    ...
    fn the_same_tone_lands_in_the_band_that_contains_it() {
        ...
        assert!(band_power(&x, FS, BETA) > band_power(&x, FS, ALPHA) * 100.0);
    }
    ...
    fn power_scales_with_the_square_of_amplitude() {
        ...
        let n = 2048;
        let p1 = band_power(&sine(10.0, 1.0, n), FS, ALPHA);
        let p2 = band_power(&sine(10.0, 2.0, n), FS, ALPHA);
        assert!(
            (p2 / p1 - 4.0).abs() < 0.01,
            "expected 4x, got {:.4}x",
            p2 / p1
    ...
    fn a_constant_signal_has_no_band_power_because_the_mean_is_removed() {
        ...
        assert!(band_power(&x, FS, ALPHA) < 1e-20);
    }

    #[test]
    fn under_one_second_of_data_refuses_rather_than_estimating() {
        // Mirrors the Python guard exactly: len < int(fs) returns 0.
        let x = sine(10.0, 20.0, 255);
        assert_eq!(band_power(&x, FS, ALPHA), 0.0);
        // At exactly one second it computes.
        let y = sine(10.0, 20.0, 256);
        assert!(band_power(&y, FS, ALPHA) > 0.0);
    }

    #[test]
    fn a_non_finite_sample_returns_zero_rather_than_propagating_nan() {
        ...
        y[7] = f64::INFINITY;
        assert_eq!(band_power(&y, FS, ALPHA), 0.0);
    }
    ...
    fn both_band_edges_are_inclusive() {
        ...
        let inclusive = band_power(&top, FS, (8.0, 13.0));
        let excluded = band_power(&top, FS, (8.0, 12.9));
        ...
        let inclusive = band_power(&bottom, FS, (8.0, 13.0));
        let excluded = band_power(&bottom, FS, (8.1, 13.0));
    ...
    #[test]
    fn an_empty_or_degenerate_band_returns_zero() {
        let x = sine(10.0, 20.0, 2048);
        assert_eq!(band_power(&x, FS, (13.0, 8.0)), 0.0); // inverted
        assert_eq!(band_power(&x, FS, (1000.0, 2000.0)), 0.0); // above Nyquist
        assert_eq!(band_power(&x, 0.0, ALPHA), 0.0); // invalid rate
    }
    ...
    fn it_agrees_with_the_python_reference_to_twelve_significant_figures() {
        ...
        for c in cases {
            let got = band_power(&sine(c.freq, c.amp, c.n), FS, c.band);
